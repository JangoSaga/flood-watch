  ...
  const handlePredict = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const risk = d?.flood_prediction?.risk
        const status = risk === 1 ? "Unsafe" : "Safe"
        const wa = d?.weather_analysis || {}
        setPrediction({
          city: selectedCity,
          status,
          temperature: wa.temperature_avg ?? d?.temp ?? 0,
          maxTemperature: wa.temperature_max ?? d?.maxt ?? 0,
          windSpeed: wa.wind_speed ?? d?.wspd ?? 0,
          cloudCover: wa.cloud_cover ?? d?.cloudcover ?? 0,
          precipitation: wa.precipitation_total ?? d?.percip ?? 0,
          humidity: wa.humidity ?? d?.humidity ?? 0,
          confidence: d?.flood_prediction?.confidence ?? d?.confidence ?? undefined,
        })
      } else {
